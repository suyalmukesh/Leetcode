-- https://leetcode.com/problems/get-the-second-most-recent-activity/description/?envType=study-plan&id=sql-iii
...
(
    select username , activity , startdate , enddate ,
           row_number() over (partition by username order by enddate desc)R,
           count(username) over (partition by username )RR
    from useractivity
)    
select  username , activity , startdate , enddate
from cte 
...
(
    select username , activity , startdate , enddate ,
           row_number() over (partition by username order by enddate desc)R,
           count(username) over (partition by username )total_activity
    from useractivity
)    
select  username , activity , startdate , enddate
from cte 
where (R = 2 and total_activity > 1) or (R = 1 and total_activity = 1);
