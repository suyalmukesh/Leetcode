-- https://leetcode.com/problems/countries-you-can-safely-invest-in/?envType=study-plan&id=sql-ii
...
-- Solution 1 
-- The below Solution worked for Sample Question but failed while submitting 

with cte as 
(
    select a.id,b.name from person a 
    inner join country b 
    on substr(a.phone_number,1,3) = b.country_code
),
total as
(
select (2*sum(duration))/(2*count(duration))as total_sum from calls
),
avgg as 
(
select caller_id,2*sum(duration)S,2*count(duration)C  from calls group by caller_id order by caller_id
)
select distinct(cte.name) as country from cte
inner join avgg on cte.id = avgg.caller_id
and avgg.S/avgg.C > (select total_sum from total)
