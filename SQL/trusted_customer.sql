...
with cte as 
( select user_id , count(user_id)as contacts_cnt from contacts group by user_id ),

trusted as
(  select user_id , count(user_id)trusted_cnt from 
    (  select user_id,contact_name from Contacts a
        inner join customers b on a.contact_name = b.customer_name  
    )aa 
    group by user_id
)
select a.invoice_id ,
       ...
       ifnull(trusted.trusted_cnt,0) as trusted_contacts_cnt 
from invoices a 
inner join customers b on a.user_id = b.customer_id 
left join cte on a.user_id = cte.user_id
left join trusted on a.user_id = trusted.user_id
